...
exports.uploadTint = (req, res) => {
	...
	conn.query(
		'INSERT INTO Tints(title, content, video_id, user_id, like_cnt, category) VALUES(?, ?, ?, ? ,? ,?)',
		[title, content, video_id, req.decoded._id, 0, category],
		(err, result) => {
			if (err) throw err;
...
exports.getTintList = (req, res) => {
	if (req.query.category == 0) {
		conn.query(
			'SELECT Tints.title, Tints.content, Tints.video_id, Users.id, Users.username, Tints.like_cnt FROM Tints JOIN Users on Tints.user_id = Users.id',
			(err, result) => {
				if (err) throw err;
				return res.status(200).json({
					result
